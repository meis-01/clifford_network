...
def fftnd(image: np.ndarray, axes: Optional[Sequence[int]] = [-1]) -> np.ndarray:
    """
    Compute the centered n-dimensional Fourier transform with unitary scaling.

    Parameters:
    -----------
    image: np.ndarray
        The input image-space data.
    axes: list or tuple, optional
        The list of axes along which to compute the Fourier transform. Default is [-1].

    Returns:
    --------
    kspace: ndarray
        The output k-space array after the centered Fourier transform.
    """

    if axes is None:
        axes = range(image.ndim)
    kspace = fftshift(fftn(ifftshift(image, axes=axes), axes=axes), axes=axes)
