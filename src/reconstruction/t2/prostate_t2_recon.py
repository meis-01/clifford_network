...
from src.data.mri_data import zero_pad_kspace_hdr
from src.reconstruction.utils import center_crop_im, ifftnd
...
if __name__ == "__main__":
    ...
    path = r"C:\Users\meisa\Projects\clifford_network\notebooks\data\file_prostate_AXT2_001.h5"
    with h5py.File(path, "r") as f:
        try:
            kspace = f["kspace"][:]  # shape: (averages, coils, H, W)
        except:
            kspace = None
        try:
            calib = f["calibration_data"][:]
        except:
            calib = None
        try:
            ismri = f["ismrmrd_header"][()]
        except:
            ismri = None
        try:
